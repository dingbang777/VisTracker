...
class BehaveDatasetOnline(BehaveDataset):
    def __init__(self, data_paths, batch_size, phase,
        ...
        for path in data_paths:
            date = DataPaths.get_seq_date(path)
            seq_roots.setdefault(date, osp.dirname(osp.dirname(path)))
        seq_dates = sorted(seq_roots)
        self.kin_transforms = {
            date: KinectTransform(seq_roots.get(date, date_seqs[date]), no_intrinsic=True) for date in seq_dates
        }
