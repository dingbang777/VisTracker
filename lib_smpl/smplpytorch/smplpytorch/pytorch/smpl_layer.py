...
class SMPL_Layer(Module):
    ...
    def __init__(self,
        ...
        self.register_buffer(
            'th_J_regressor',
            torch.Tensor(np.asarray(j_regressor)))
    ...
    def _match_betas(self, th_betas):
        """Pad or truncate betas to match the loaded model's shape space."""
        target_dim = self.th_shapedirs.shape[-1]
        if th_betas.shape[1] == target_dim:
            return th_betas
        if th_betas.shape[1] > target_dim:
            return th_betas[:, :target_dim]
        pad = th_betas.new_zeros((th_betas.shape[0], target_dim - th_betas.shape[1]))
        return torch.cat([th_betas, pad], dim=1)
    ...
    def forward(self,
        ...
        if th_betas is None:
            ...
            th_betas = self._match_betas(th_betas)
