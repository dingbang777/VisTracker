...
from utils.custom_sequence import get_pixel_camera_params, use_custom_video
...
class BaseFitter:
    def __init__(self, device='cuda:0', debug=False, init_type='mocap', args=None):
        ...
        if args is not None and use_custom_video(args):
            camera_params = get_pixel_camera_params(args.seq_folder, args.kid)
            self.fx = camera_params["fx"]
            self.fy = camera_params["fy"]
            self.cx = camera_params["cx"]
            self.cy = camera_params["cy"]
            print("Using custom video camera parameters:", camera_params)
    ...
    def fit_seq(self, seq_folder, kid, start, end, redo, bs=512):
        ...
        if self.icap:
            ...
            if not use_custom_video(self.args):
    ...
    def check_frame_consistency(self, packed_data, seq_folder):
        ...
        frames_all = sorted(glob(seq_folder + "/t*/"))
