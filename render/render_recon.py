...
from utils.custom_sequence import use_custom_video
...
class RendererBase:
    ...
    def render_seq(self, args):
        ...
        temp = load_template(seq_info.get_obj_name(), seq_folder=args.seq_folder if use_custom_video(args) else None)
        ...
        video_writer = imageio.get_writer(
            video_path, format='FFMPEG', mode='I', fps=args.fps, macro_block_size=1
        )
        video_top = imageio.get_writer(
            video_path.replace('.mp4', "_top.mp4"), format='FFMPEG', mode='I', fps=args.fps, macro_block_size=1
        )
        video_top_cv = None
    ...
    def get_parser():
        ...
        parser.add_argument('-cv', '--custom_video', default=False, action='store_true')
