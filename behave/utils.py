...
from utils.custom_sequence import get_custom_template_path
...
def load_template(obj_name, cent=True, high_reso=False, behave_path=BEHAVE_ROOT, seq_folder=None):
    "load object template mesh given object name"
    if seq_folder is not None:
        temp_path = get_custom_template_path(seq_folder, obj_name)
    else:
        temp_path = get_template_path(behave_path+"/objects", obj_name)
    if high_reso:
        assert seq_folder is None, "custom video does not support high_reso template loading"
